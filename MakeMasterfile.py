...
all_participants = ['mode','area','bird','hall','user','oven','army','road',  \
                    'cell','poem','food','town','year','news','goal','week',  \
                    'mall','beer','gate','gene','desk','unit','disk','meat',  \
                    'king','debt','idea','soup','city','girl','dirt','role',  \
                    'poet','song','fact','lake','bath','nice','path','bite', \
                    'loan','chat','zone','zeal']

moves = {'AfterTheRain':496,'BetweenViewings':808,'BigBuckBunny':490,'Chatter':405,\
         'FirstBite':599,'LessonLearned':667,'Payload':1008,\
         'Sintel':722,'Spaceman':805,'Superhero':1028, \
         'TearsOfSteel':588,'TheSecretNumber':784,'ToClaireFromSonny':402,'YouAgain':798 #,'RidingTheRails':794,'DamagedKungFu':922
         }

its = ['Standards','PleasantSelf','SocialNorms','PleasantOther','GoalsOther', \
       'Controlled','Predictable','Suddenly','Agent','Urgency', \
        'Lips','Tears','Eyebrows','Smile','Frown','Stop','Undo','Repeat','Oppose','Attention','Tackle', \
        'Command','Support','Move','Care', 'Bad', 'Good','Calm','Strong','IntenseEmotion','Alert', \
        'AtEase','Muscle','Heartrate','Throat', \
        'Stomach','Warm','Anger','Guilt','WarmHeartedness', \
        'Disgust','Happiness','Fear','Regard','Anxiety', \
        'Satisfaction','Pride','Surprise','Love','Sad' #,'Jaw', 'EyesOpen', 'Breathing', 'Movement','Consequences'
        ]
...
numberAnnot = {'3':0, '4':0,'_3':0,'_4':0} # number of annotations making a ground truth '_' marks that one was removed for agreement/five
zfiles = True
saving = False  
    ...
    for p in all_participants: # Loop over participants to find files
        for n in its: # Loop over items
            ...
            for mix, movie in enumerate(moves): # Loop over films
                ...
                files = glob.glob(f'{root}Annotstudy/sub-{p}/beh/sub-{p}_task-{movie}_recording-{n}_stim.tsv.gz')
                if len(files) > 4:
            ...
            for num,val in enumerate(val_films):
                ...
                np.savetxt(f"{temp}sub-{p}_task-{val}_recording-{n}_stim.tsv", zdata, fmt="%.6f", delimiter='\t')
...
for mix, movie in enumerate(moves): # Loop over films
    for iix, n in enumerate(its): # Loop over items
        ...
        for six, p in enumerate(all_participants):  # Loop over participants to find files
        ...
        if group.shape[1] > 2:
            ...
            for i,j in enumerate(itertools.combinations(range(group.shape[1]),2)):
                ccc[movie + '_' + n + '_' + str(j[0]) + '_' + str(j[1]) ] = (lins_ccc(group[:,j[0]],group[:,j[1]]))                      
            
            # Calculate the mean_ccc for this filmxitem combination
            mean_ccc[mix,iix] = np.mean([ccc[z] for z in ccc.keys() if z.find(movie + '_' + n) == 0])
            ...
            for q in range(group.shape[1]):
                ccc_loolist = [ccc[z] for z in ccc.keys() if z.find(str(q)) == -1 and z.find(movie + '_' + n) >= 0]
                ccc_loo[q] = np.mean(ccc_loolist)    
            
            best_ccc = max(ccc_loo.values()) # Best CCC possible after leaving out a participant
            wr_idx = [i for i in ccc_loo if ccc_loo[i] == best_ccc][0] # Worst raters index in this filmxitem combination
            ...
            print('ALERT, this case should NOT happen when all films and items are selected')
            mean_ccc[mix,iix] = lins_ccc(group[:,0],group[:,1])
        
        if group.shape[1] <= 3: # If there are only 3 or less annotators
            if (best_ccc - mean_ccc[mix,iix]) >= threshold: # Check that there isn't a major outlier in these
                print(f'ALERT, only {str(group.shape[1])} raters left {movie}_{n}')
                print(f'Agreement is {str(mean_ccc[mix,iix])} instead of {str(best_ccc)}')
                print(np.shape(group.shape))
            numberAnnot['3'] += 1 
                
        elif group.shape[1] == 4: # Standard case is if there are 4 annotators
            if (best_ccc - mean_ccc[mix,iix])>= threshold: # Check if exclusion would make the mean better
                # Erase worst rater from everything if True
                for i in [z for z in ccc.keys() if z.find(str(wr_idx)) != -1 and z.find(movie + '_' + n) == 0]:
                    del ccc[i] 
                ...
                numberAnnot['4'] += 1 
        elif group.shape[1] == 5: # In the rare case of 5 annotators we remove the worst in any case
...
sn.heatmap(mfilm.T, square = True, xticklabels=its, yticklabels=moves,cmap ="coolwarm",center = 0)
fig.savefig('/Volumes/Sinergia_Emo/EPFL_drive/Sinergia Project/Writing/Data_Paper/Figures/Figure2.png', bbox_inches='tight')
