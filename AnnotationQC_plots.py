...
savedir =  '/Volumes/Sinergia_Emo/EPFL_drive/Sinergia Project/Writing/Data_Paper/Figures/'
...
all_participants = ['mode','area','bird','hall','user','oven','army','road',  \
                    ...
                    'loan','chat','zone','zeal']
all_movies = ['AfterTheRain','BetweenViewings','BigBuckBunny','Chatter', \
              'DamagedKungFu','FirstBite','LessonLearned','Payload', \
              'RidingTheRails','Sintel','Spaceman','Superhero', \
              'TearsOfSteel','TheSecretNumber','ToClaireFromSonny','YouAgain']

durs = [496, 808, 490, 405, 922, 599, 667, 1008, 794, 722, 805, 1028, 588, 784, 402, 798]

all_items = ['Standards','PleasantSelf','SocialNorms','PleasantOther','GoalsOther', \
             'Controlled','Predictable','Suddenly','Agent','Urgency', \
             'Consequences','Lips','Tears','Jaw','Eyebrows', \
             'Smile','Frown','EyesOpen','Movement','Stop', \
             'Undo','Repeat','Oppose','Attention','Tackle', \
             'Command','Support','Move','Care', 'Bad', \
             'Good','Calm','Strong','IntenseEmotion','Alert', \
             'AtEase','Muscle','Heartrate','Breathing','Throat', \
...
counts, bns, patches = plt.hist(ccc, bins=bins, color='darkblue', alpha = 0.8, edgecolor='black')
...
ax0.barh(bns[:-1], counts, height=widths, align='center', color='darkblue', alpha = 0.8, edgecolor='black')
...
# initializing x,y axis value
bins = np.arange(-0.2, 0.9, .05)

ax0 = fig.add_subplot(spec[0])
boxplot = ax0.boxplot(file, 
                    patch_artist=True, 
                    labels = ilist, 
                    showmeans=True,
